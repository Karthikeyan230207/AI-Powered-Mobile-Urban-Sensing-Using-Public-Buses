...
export default function RecentIncidents({ incidents = [], loading, error, onSelect }) {
  ...
  return (
    <section className="panel recent-incidents">
      ...
      {error && <div className="state-message">Backend unavailable. Start the FastAPI server to load live incidents.</div>}
      {!loading && !error && items.length === 0 && <div className="state-message">No incidents found.</div>}
